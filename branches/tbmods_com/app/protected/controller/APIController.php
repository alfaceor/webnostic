...
class APIController extends DooController{
    ...
    function getSampleJob(){
        ...
        if(trim($signature)==trim($generated_signature)){
			...
			$query_result = $sample->getOne(array("where" => "user_id LIKE '%".$this->params['userlab']."|%'"));
            ...
            $img_filename = IMAGE_PATH_SAMPLE."/".$this->params['userlab']."/".$query_result->imagename;
    ...
    function putSampleResult(){
        ...
        if(trim($signature)==trim($generated_signature)){
            ...
            //$this->sendResultMail($sample_id,$score,$result); --------- not send mail for each job ----------------------------------------------
    ...
    function sendResultMail($sample_id,$score,$result){
        ...
        Doo::loadModel('User');
        $user   = new User();
        $user->username   = $query_result_samp->user_id;
        $query_result_user = $user->getOne();
        print_r($query_result_user) ;
        
        //$to = $query_result_user->correo;
        $to      = 'betollaque@gmail.com';
        $subject = Doo::conf()->SITE.'- software result '.$sample_id;
        $message = 'sample id: '.$sample_id.', score: '.$score.', result: '.$result.' --- de:'.$query_result_user->correo;
